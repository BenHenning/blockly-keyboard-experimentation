...
suite('Workspace comment navigation', function () {
  ...
  setup(async function () {
    ...
    [this.commentId1, this.commentId2] =
    ...
    this.getCommentLocation = async (commentId: string) => {
      return this.testDriver.browser.execute((commentId: string) => {
  ...
  test('Navigate forward from workspace comment to block', async function () {
    await this.testDriver.focusOnWorkspaceComment(this.commentId2);
    await this.testDriver.keyDown();
    const focusedBlock = await this.testDriver.getFocusedBlockType();
    chai.assert.equal(focusedBlock, 'p5_draw');
  });

  test('Navigate backward from block to workspace comment', async function () {
    await this.testDriver.focusOnBlock('p5_draw_1');
  ...
  test('Navigate backward from workspace comment to block', async function () {
    await this.testDriver.focusOnWorkspaceComment(this.commentId1);
    await this.testDriver.keyUp();
    const focusedBlock = await this.testDriver.getFocusedBlockType();
  ...
  test('Activate workspace comment button', async function () {
    ...
    const collapsed = await this.testDriver.browser.execute((commentId) => {
  ...
  test('Terminating editing commits edits and focuses root workspace comment', async function () {
    await this.testDriver.focusOnWorkspaceComment(this.commentId1);
    await this.testDriver.sendKeyAndWait(Key.Enter);
    await this.testDriver.sendKeyAndWait('Hello world');
    await this.testDriver.sendKeyAndWait(Key.Escape);
    const focusedNodeId = await this.testDriver.getCurrentFocusNodeId();
    chai.assert.equal(focusedNodeId, `${this.commentId1}`);

    const commentText = await this.testDriver.browser.execute((commentId) => {
  ...
  test('Action menu can be displayed for a workspace comment', async function () {
    ...
    chai.assert.isTrue(
      await this.testDriver.contextMenuExists('Duplicate Comment'),
      'The menu should be openable on a workspace comment',
    );
    chai.assert.isTrue(
      await this.testDriver.contextMenuExists('Remove Comment'),
      'The menu should be openable on a workspace comment',
    );
    chai.assert.isTrue(
      await this.testDriver.contextMenuExists('Move CommentM'),
      'The menu should be openable on a workspace comment',
    );
  });
  ...
  test('Workspace comments can be moved in unconstrained mode', async function () {
    ...
    await this.testDriver.sendKeyAndWait([Key.Alt, Key.ArrowDown], 2);
